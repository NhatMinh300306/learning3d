...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        #   - xy_grid: torch.Tensor of shape (W * H, 2) - Tọa độ của các pixel trong không gian NDC
        #   - ray_bundle: RayBundle chứa origins (W * H, 3) và directions (W * H, 3)
        ...
        # shapes/types:
        #   - ray_bundle: RayBundle sau khi qua sampler sẽ có sample_points (W * H, N_pts, 3) và sample_lengths (W * H, N_pts, 1)
        ...
        # shapes/types:
        #   - out: dict chứa 'feature' (W * H, 3) và 'depth' (W * H, 1)
        ...
        # shapes/types:
        #   - depth: out['depth'] có shape (W * H, 1) - Bản đồ độ sâu được tích hợp của các tia
...
def train(
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            # Sample rays
            # shapes/types:
            #   - xy_grid: torch.Tensor of shape (batch_size, 2) - Tọa độ pixel lấy mẫu ngẫu nhiên trên không gian NDC
            ...
            # shapes/types:
            #   - rgb_gt: torch.Tensor of shape (batch_size, 3) - Màu sắc thực tế từ ảnh
            #   - out['feature']: torch.Tensor of shape (batch_size, 3) - Màu sắc dự đoán từ mô hình
            #   - loss: scalar torch.Tensor - L2 loss giữa màu sắc thực tế và dự đoán
            loss = loss = torch.nn.functional.mse_loss(rgb_gt, out['feature'])
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            # shapes/types:
            #   - rgb_gt: torch.Tensor of shape (batch_size, 3) - Màu sắc thực tế từ ảnh
            #   - out['feature']: torch.Tensor of shape (batch_size, 3) - Màu sắc dự đoán từ NeRF
            #   - loss: scalar torch.Tensor - L2 loss (MSE)
