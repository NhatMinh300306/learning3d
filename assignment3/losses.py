...
def eikonal_loss(gradients):
    ...
    # shapes:
    #   - gradients: (N, 3) - Vector gradient của hàm SDF đối với tọa độ điểm 3D
    #   - returns loss: scalar torch.Tensor - Giá trị Eikonal loss (độ lớn gradient tiệm cận 1)
    # TODO (Q6): Implement eikonal loss
    loss = ((gradients.norm(2, dim=1) - 1.0) ** 2).mean()
    return loss
