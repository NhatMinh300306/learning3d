...
def get_pixels_from_image(image_size, camera):
    # type: (Union[Tuple[int, int], List[int]], CamerasBase) -> torch.Tensor
    # shapes:
    #   - image_size: (width, height)
    #   - returns xy_grid: (W * H, 2) - Tọa độ của các pixel trong không gian NDC [-1, 1]
    W, H = image_size[0], image_size[1]

    # TODO (Q1.3): Generate pixel coordinates from [0, W] in x and [0, H] in y
    # shapes: x is (W,), y is (H,)
    x = torch.arange(W)
    y = torch.arange(H)
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    # type: (int, Union[Tuple[int, int], List[int]], CamerasBase) -> torch.Tensor
    # shapes:
    #   - n_pixels: Số lượng pixel ngẫu nhiên cần lấy (batch size)
    #   - image_size: (width, height)
    #   - returns xy_grid_sub: (n_pixels, 2) - Tọa độ pixel ngẫu nhiên trong không gian NDC
    xy_grid = get_pixels_from_image(image_size, camera)
    
    # TODO (Q2.1): Random subsampling of pixel coordinaters
    indices = torch.randperm(xy_grid.shape[0], device=xy_grid.device)[:n_pixels]
    
    # Lọc ra các tọa độ tương ứng với các chỉ mục đã chọn ngẫu nhiên
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    # type: (torch.Tensor, Union[Tuple[int, int], List[int]], CamerasBase) -> RayBundle
    # shapes:
    #   - xy_grid: (N_pixels, 2) - Tọa độ pixel trên NDC space
    #   - image_size: (width, height)
    #   - returns RayBundle: chứa origins (N_pixels, 3) và directions (N_pixels, 3)
    W, H = image_size[0], image_size[1]

    # TODO (Q1.3): Map pixels to points on the image plane at Z=1
    # ndc_points: Tập tọa độ các pixels
    # shapes: ndc_points is (N_pixels, 3)
    ndc_points = xy_grid
    ...
    # shapes: image_plane_points is (N_pixels, 3)
    ...
    # rays_o: Điểm bắt đầu tia sáng -> trả về tensor chứa tọa độ bắt nguồn các tia sáng chiếu vào pixel
    # shapes: rays_o is (N_pixels, 3)
