...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        with torch.no_grad():
            # FIX: torch.cuda.empty_cache() có thể lỗi nếu chạy CPU-only / không có CUDA
            ...
            if lights is not None:
                # shapes/types:
                #   - light_location: torch.Tensor of shape (3,) hoặc (1, 3) - Tọa độ nguồn sáng
                #   - origin: torch.Tensor of shape (3,) - Điểm gốc tọa độ (vị trí đặt vật thể)
                #   - light_dir: torch.Tensor of shape (1, 3) - Hướng chiếu của ánh sáng
...
def create_model(cfg):
    ...
    if len(cfg.training.checkpoint_path) > 0:
        ...
        if cfg.training.resume and os.path.isfile(checkpoint_path):
            ...
            # FIX: map_location='cpu' để load checkpoint được lưu từ GPU trên máy không có GPU
...
def train_points(
    ...
    # FIX: .cuda() -> .cpu()
    all_points = torch.Tensor(point_cloud["verts"][::2]).cpu().view(-1, 3)
    ...
    for epoch in range(0, cfg.training.num_epochs):
        ...
        for idx in t_range:
            ...
            # shapes/types:
            #   - points: torch.Tensor of shape (batch_size, 3) - Các điểm lấy mẫu ngẫu nhiên từ point cloud
            #   - distances: torch.Tensor of shape (batch_size, 1) - Khoảng cách SDF dự đoán tại các điểm này
            #   - gradients: torch.Tensor of shape (batch_size, 3) - Gradient của khoảng cách đối với tọa độ điểm
            #   - loss: scalar torch.Tensor - L2 hoặc L1 loss ép khoảng cách tại point cloud tiến về 0
            ...
            eikonal_points = get_random_points(
                cfg.training.batch_size, cfg.training.bounds, 'cpu'
            )

            # Get sdf gradients and enforce eikonal loss
            # shapes/types:
            #   - eikonal_points: torch.Tensor of shape (batch_size, 3) - Các điểm mẫu ngẫu nhiên trong không gian hộp bao
            #   - eikonal_distances: torch.Tensor of shape (batch_size, 1) - SDF tại các điểm Eikonal
            #   - eikonal_gradients: torch.Tensor of shape (batch_size, 3) - Gradient của SDF tại các điểm Eikonal
            #   - loss: scalar torch.Tensor - Giá trị loss tổng thể bao gồm Point loss, Eikonal loss, và Inter loss
...
def train_images(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            # Get sdf gradients and enforce eikonal loss
            # shapes/types:
            #   - eikonal_points: torch.Tensor of shape (batch_size, 3)
            #   - eikonal_distances: torch.Tensor of shape (batch_size, 1)
            #   - eikonal_gradients: torch.Tensor of shape (batch_size, 3)
